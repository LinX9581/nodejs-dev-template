...
router.post("/api/chart-data", logUserAccess("GitHub 熱門成長專案查詢"), asyncHandler(async (req, res) => {
  ...
  try {
    const data = await requestJson(`https://api.github.com/search/repositories?q=created:>${dateString}&sort=stars&order=desc&per_page=20`, {
    ...
    return res.status(statusCode).json({
      status: "error",
      message: `GitHub API 請求失敗: ${error?.code || error?.message || "UNKNOWN_ERROR"}`,
...
router.post("/api/popular-repos", logUserAccess("熱門專案數據查詢"), asyncHandler(async (req, res) => {
  try {
    const data = await requestJson("https://api.github.com/search/repositories?q=stars:>10000&sort=stars&order=desc&per_page=30", {
      headers: GITHUB_API_HEADERS
    });
